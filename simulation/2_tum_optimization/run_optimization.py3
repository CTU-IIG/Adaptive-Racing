...
"""
Based on a script 'main_globaltraj.py' created by:
Alexander Heilmeier
...
PARSER = argparse.ArgumentParser(
    prog = "run_optimization",
    formatter_class=argparse.RawDescriptionHelpFormatter,
    description = """
Generate an optimal trajectory based on a given reference track.
    """,
)
...
PARSER.add_argument(
    ...
    help = "Path to file containing vehicle parameters.",
    type = str,
)
...
file_paths = {"veh_params_file": args.model}

# debug and plot options -----------------------------------------------------------------------------------------------
debug = args.debug                              # print console messages
...
file_paths["inputs"] = args.inputs_folder

# set plot stem
if args.save_plot:
    plot_opts["output_stem"] = os.path.join(
        file_paths["module"], "outputs", args.output
    )
...
file_paths["track_file"] = file_paths["track_name"] + ".csv" # os.path.join(file_paths["module"], "inputs", "tracks", file_paths["track_name"] + ".csv")

# assemble friction map import paths
file_paths["tpamap"] = os.path.join(file_paths["inputs"], "inputs", "frictionmaps",
                                    file_paths["track_name"] + "_tpamap.csv")

if mintime_opts["tpadata"] is None:
    file_paths["tpadata"] = os.path.join(file_paths["inputs"], "inputs", "frictionmaps",
                                         file_paths["track_name"] + "_tpadata.json")
else:
    file_paths["tpadata"] = os.path.join(file_paths["inputs"], "inputs", "frictionmaps", mintime_opts["tpadata"])
...
file_paths["traj_race_export"] = os.path.join(file_paths["module"], "outputs", args.output + ".csv")
...
if not parser.read(file_paths["veh_params_file"]): #os.path.join(file_paths["module"], "params", file_paths["veh_params_file"])):
...
if not (opt_type == 'mintime' and not mintime_opts["recalc_vel_profile_by_tph"]):
    file_paths["ggv_file"] = os.path.join(file_paths["inputs"], "inputs", "veh_dyn_info", pars["ggv_file"])
    file_paths["ax_max_machines_file"] = os.path.join(file_paths["inputs"], "inputs", "veh_dyn_info",
...
if args.save_plot:
    pars_tmp["output_stem"] = os.path.join(
        file_paths["mintime_export"], args.output
    )
...
if opt_type == 'mincurv':
    ...
    alpha_opt, reftrack_interp, normvec_normalized_interp, spline_len, psi, kappa, dkappa = tph.iqp_handler.\
        iqp_handler(reftrack=reftrack_interp,
                    spline_len = None, psi = None, kappa = None, dkappa = None,
                    normvectors=normvec_normalized_interp,
                    A=a_interp,
                    kappa_bound=pars["veh_params"]["curvlim"],
                    w_veh=pars["optim_opts"]["width_opt"],
...
if plot_opts["racetraj_vel"]:
    ...
    #plt.show()
    plt.savefig(
        os.path.join(file_paths["module"], "outputs", args.output + "_velocity.png")
    )
...
if "traj_race_export" in file_paths.keys():
    ...
    print(traj_race_cl.shape)
    with open(file_paths["traj_race_export"], "w") as f:
        np.savetxt(
            f, traj_race_cl, fmt = "%.7f,%.7f,%.7f,%.7f,%.7f,%.7f,%.7f", header = "d_m,x_m,y_m,psi_rad,k_radpm,v_mps,a_mps2", comments = ""
        )
